...
data_ls = [
    "de.csv",
   # "euro600.csv",
    "eu_euro_notated_ex_germany.csv",
    "eu_not_euro_notated.csv"
]
...
if test:
    for datalist in data_ls:
        ...
        fun_process_stocks_new(stockinfo_pkl[:20], datalist)
        time.sleep(1)
else:
      for datalist in data_ls:
...
#data_input_euro600 =  pd.read_pickle("screener/screened_data/proc_euro600.csv.pkl")
...
data_eu.drop_duplicates(subset=["Ticker"], inplace=True)
data_eu.to_pickle("screener/screened_data/proc_eu.csv.pkl")
...
for proc_data in proc_ls:
    if proc_data in ["proc_all.csv.pkl", "proc_eu.csv.pkl"]:
